...
public class LinkedQueue<E> implements Cloneable
{
   ...
   **/ 
   ...
   //
   //Peek method will return the item from the front of the queue without removing it from the queue
   //Preconditon: A queue must be created
   //Postcondition: Method will return the front item of the queue
   //
   public E peek( ){
	  E answer;
      if (manyNodes == 0)
   ...
   //
   //updateFront Method will replace the element at the front of the queue with the given item
   //Precondition: Valid element to replace the front item of the queue
   //Postcondition: The front item of the queue will be changed to the requested element
   //
